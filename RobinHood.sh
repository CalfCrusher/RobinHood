...
python3 $DIRSEARCH -l live_subdomains_$HOST.txt --max-time=86400 -e php,aspx,txt,sql,bak -w $DIRSEARCH_WORDLIST -q -t 10 -i 200,307,301,302,401 -o dirsearch_results.txt --format=plain
...
$KATANA -u live_subdomains_$HOST.txt -d 3 -ef png,jpg,gif,jpeg,swf,woff,svg,pdf,tiff,tif,bmp,webp,ico,mp4,mov,js,css,eps,raw -kf -jc -ct 1800 -c 5 -p 2 -rl 50 | tee -a all_urls_$HOST.txt
...
# Get php endpoints
cat live_urls_$HOST.txt | grep ".aspx" | cut -f1 -d"?" | sed 's:/*$::' | sort -u > aspx_endpoints_urls_$HOST.txt
...
then
    ...
    # Collect hidden params with Arjun
    $ARJUN -i aspx_endpoints_urls_$HOST.txt -o arjun_asp_GET_results.txt -m GET
    $ARJUN -i aspx_endpoints_urls_$HOST.txt -o arjun_asp_POST_results.txt -m POST
fi
